...
MCP server platform exposing D365FO metadata as 75 AI-consumable tools across 5 services (KB 32, XRef 18, Security 19, Labels 4, Task Recorder 2). 69 read a local snapshot; 4 read sealed-ISV metadata; 1 (`d365_custom_fields`) reads a live D365 environment; 1 (`d365_knowledge`) serves a curated rulebook shipped with the code.

## Key Commands

```bash
npm install                    # Install dependencies
npm test                       # Type check (tsc, checkJs) + all tests — the checker baseline is 0 since 2026-09-09 (#103) and is a CI job
npm run test:only              # All tests without the type check
npm run typecheck              # tsc -p jsconfig.json (noEmit; JSDoc types over plain JS, no .ts anywhere)
...
- `src/azure/` — Shared tool implementations (used by both local and Azure)
  ...
  - `kb-tools.js` — 17 KB tools, exports `registerKbTools(server, db)`; its last lines call `registerEffectiveSchemaTools(server, db)` and `registerAuthoringTools(server, db)` so `tool-sets.js` needs no change. **Labels v2 (#84/#127)** lives here: `d365_search(object_type: 'label')` searches label TEXT (FTS5 `labels_fts`, LIKE fallback) and `d365_resolve_label(languages[])` returns one row per (id, language); both detect the v2 columns once per registration and serve a pre-v2 snapshot unchanged (rule #14: `language` on every row or on none)
  - `authoring-tools.js` — the six **authoring-loop read tools** (#123–#128, added 2026-09-03 after the comparison with dynamics365ninja/d365fo-mcp-server — the read side of their write loop, never the writes): `d365_find_method_implementations` ("how do others implement validateWrite", tier 1, needs `idx_methods_name_nocase`), `d365_lookup_object` (queries, reports, maps, macros, config keys, services, service groups, security policies, menus from `objects_meta`), `d365_lookup_form` / `d365_find_forms` (design pattern + version, control tree with data binding from `form_controls`, pattern catalogue, clone-from by pattern × table), `d365_preflight` (exists · module/origin · sealed-ISV · method signature parsed by `analyzeSignature()` against the Microsoft wrappability table · existing extension classes + `isv_coc` wrappers · `proposed_names` collisions across `object_paths` ∪ `menu_items` ∪ `objects_meta` ∪ `isv_elements` · prefix rule from `KB_NAMING_PREFIXES`), `d365_knowledge` (curated X++ rulebook in `src/azure/knowledge/*.md` — frontmatter `topic/title/aliases/tags/sources` — loaded lazily from the CODE, not the snapshot, so connector clients get what the plugin skills carry). Every tool degrades on a pre-1.2 snapshot (`patterns_indexed: false`, an error naming the rebuild) — a missing table is a coverage fact, not a crash
  - `kb-indexes.js` — `KB_INDEXES` + `ensureKbIndexes(dbPath)`, the KB twin of `sec-indexes.js`: `getKbDb()` and `src/local/mcp-server-kb.js` run it before opening read-only (`KB_AUTO_INDEX=false` disables), so an existing snapshot gets `idx_methods_name_nocase` etc. at first start; indexes whose table/column the snapshot lacks are skipped. `test/kb-indexes.test.js` asserts every statement is also in `build-kb.js`
  ...
  - `xref-tools.js` — 17 XRef tools (incl. `xref_check_exists`, #118), exports `registerXrefTools(server, db, { semanticDb }?)`
  - `sec-tools.js` — 19 Security tools (incl. `sec_check_exists`, #118), exports `registerSecTools(server, db, { semanticDb }?)`
  - `tool-guards.js` also hosts the **semantic side channel of `functional_context`** (#115): `semanticStore()` (lazy, per-process handle — never opened at registration) and `recordContextHint()`; the 9 read tools that take `functional_context` (`d365_lookup_table`, `d365_search`, `d365_get_class_methods`, `d365_get_entity_sources`, `d365_effective_schema`, `xref_find_references`, `xref_object_summary`, `sec_object_access`, `sec_lookup_role`) record a `context_hint` mapping on success and pass the id to `notFoundResult` on a miss. Every `notFoundResult` call in the tool files passes `{ db, kind }` so a near-miss name is always recovered
...
4. **Freshness banner (shipped 2026-09-02, #86 base; per-model age 2026-09-09, #86 items 1–4):** every DATA response of a snapshot-backed tool carries `_<Service> snapshot: <YYYY-MM-DD> (<N> days old)_` (`_KB snapshot: 2026-08-14 (26 days old)_`, `_XRef …_`, `_Sec …_`; the age qualifier is omitted on the build day, and the local XRef server adds `live refresh` when `XREF_LIVE=1`) on the line **directly after the H2 heading** — not before it, so rule #3 (the response opens with H2) still holds. Age is whole UTC days computed by `snapshotAgeDays()`; tests pin "today" with `setFreshnessClock()`. **Per-model freshness** lives in `model_versions.indexed_at` (`MODEL_VERSIONS_SCHEMA`; `insertModelVersions` stamps one instant per build, `ensureModelVersionsColumns()` adds the column to a pre-2026-09-09 database before the KB delta merge or the XRef module delta writes into it — the XRef delta stamps only the refreshed module's models, the KB merge carries the delta's own stamp). `queryModelVersions()` serves `indexed_at` on every row when the column exists and the old shape otherwise, so `d365_list_modules` / `d365_get_module_summary` / `xref_list_modules` / `d365://modules` emit the key on every row or on none (rule #14). `stale_search_models` from the original #86 text was NOT built: the kb_search-stale-for-extended-base-tables defect it would have reported was fixed at the source (`reindexExtendedBaseObjects()` in `merge-kb-custom.js`), so there is nothing to signal; `partial_build` (rule #16) remains the delta-merge caveat. It is attached **centrally**, not per handler: `registerServiceTools` in `tool-sets.js` wraps every registered handler with `withFreshnessBanner(result, db, service)` from `shared.js`, which calls `freshnessBanner(db, service)` (build date read once per DB handle via `readBuildDate` — tool-guards imports the same function; `''` when the snapshot is undatable, never throws). Handlers do not call it. Never on the three meta-responses: `emptyResult` / `notFoundResult` / `errorResult` stamp `_meta: { kind: 'empty' | 'not-found' | 'error' }` (part of the MCP Result type, survives the wire) and the wrapper skips any result with `_meta.kind`, any `isError`, and anything without `structuredContent`. Also skipped: live tools (`openWorldHint: true` — `d365_custom_fields` rows come from an environment now, not from a build) and db-less servers (Task Recorder). The banner goes into the `summary` text channel too (rule #5). What #86 still wants on top: per-model age (`indexed_at` in `model_versions`), an age qualifier, and the `partial_build` / `stale_search_models` caveat after a delta merge. Enforced by `test/freshness.test.js`. The identically-named function in `src/azure/wiki-storage.js` is unrelated (Wiki service, different signature).
5. **Typed-first, ADAPTIVE text channel:** any tool with an `outputSchema` builds the typed JSON object FIRST, then renders a Markdown fallback from that typed object, and returns both via `structuredResult(typed, markdownText, format)`. The text channel **defaults to `'auto'`: whichever of TOON or Markdown is smaller for that particular response.** Neither wins universally — measured on the production KB, TOON is smaller on nested payloads (`d365_get_entity_sources` 4,448 vs 5,216 tk) and *larger* on wide flat ones (`d365_lookup_table` 9,725 vs 6,772 tk, +44%, because TOON repeats a long key header). Across six real calls: TOON 21,731 / Markdown 19,653 / adaptive **18,677**. `'toon'` and `'markdown'` pin the encoding; use `'markdown'` when the text is quoted verbatim into a document. Add the shared `format: formatTextParam` to a data tool's `inputSchema` and pass the `format` arg **straight through** — never normalise it in the handler (`const fmt = format === 'markdown' ? 'markdown' : 'toon'` silently pins every default call; that bug shipped once). `structuredResult` is the choke point and treats anything that is not literally `'markdown'` or `'toon'` — including `undefined` from the test mock server — as adaptive. `structuredContent` is always the typed JSON regardless of `format`. KB / XRef / Sec data tools follow this; document/prose tools (Task Recorder, Wiki) pin `format` to `'markdown'`. Never build Markdown directly from DB rows.
   ...
   **Text-channel policy (W4, #108) — mechanism shipped, default unchanged.** Every response ships the payload **twice** — TOON/Markdown in `content[0].text` *and* JSON in `structuredContent` — and the dual send was measured at **1.3–2× the JSON alone** (20 calls, 2026-09-02: `structuredContent` is the larger channel every time; 1.3–1.6× on flat tables, ~2× on nested payloads; across six KB calls 35,927 tk JSON + 21,731 tk text = 1.60×). Run 4 found the claude.ai connector bills `structuredContent` and discards the text. `structuredResult` therefore reads the request context's `textChannel` (`src/azure/request-context.js`): **`full`** (default) is today's behaviour, byte-identical; **`summary`** makes `content[0].text` the H2 line plus one line `_Payload in structuredContent (N keys, M bytes)._` and nothing else, ≤ 300 B, `structuredContent` untouched, rule #3 still holds. Switch per request with `?text=summary` on the connector URL, the `X-MCP-Text-Channel: summary` header, or `MCP_TEXT_CHANNEL=summary` in the environment; stdio sessions additionally consult `CLIENT_TEXT_CHANNEL_POLICY` (`clientInfo.name` → channel), which is **deliberately empty**. `emptyResult` / `notFoundResult` / `errorResult` are text-only meta-responses and are never summarised. **Decision (#108, recorded 2026-09-09): option B, client-negotiated, default `full`.** Both measured clients keep exactly one channel, but not the same one: the claude.ai connector bills `structuredContent` and discards the text (Run 4, 2026-08) → it gets `?text=summary` on the connector URL; Claude Code stores `structuredContent` and never shows the model the text (C5, 2026-09-04, 10/10 results) → it gets `structured=off` (`CLIENT_STRUCTURED_POLICY` carries `claude-code: off`; the outputSchema omission needs `MCP_STRUCTURED_CONTENT=off` in the stdio entry because clientInfo arrives after registration). An unmeasured client keeps `full` (both channels, byte-identical to before W4) — the policy table grows only by measurement, never by assumption. Golden test asserts the `summary` text channel stays ≤ 300 B on every §3.1 call.

   **Structured-content policy (C5, 2026-09-04) — mechanism shipped, default unchanged.** The #108 measurement happened, the other way round: Claude Code stores `structuredContent` byte for byte and never shows the model the text channel (10/10 results of a session checked in the transcript) — so for it the TOON/Markdown work is bypassed and the JSON is what it pays for. Third request preference `structured`: **`full`** (default, byte-identical) or **`off`**, which registers every tool WITHOUT its outputSchema and strips `structuredContent` from every result after the freshness banner (`withRegistrationPolicy` → `withoutOutputSchema` / `withStructuredPolicy` in `tool-sets.js`), so the compact text — banner and coverage lines included — is the only payload. Select with `?structured=off` on the connector URL, `X-MCP-Structured-Content: off`, `MCP_STRUCTURED_CONTENT=off` in the environment, or the stdio `CLIENT_STRUCTURED_POLICY` (`clientInfo.name` → mode; carries `claude-code: off`). On stdio the clientInfo arrives after registration, so the policy table can only strip at call time — the outputSchema omission needs the env var, which the local `~/.claude.json` entries set. Test: `test/structured-policy.test.js`. The three list tools also learned to be small by default (`d365_lookup_table` `sections`, `d365_get_entity_sources` `summary`, `d365_search` `include_context`) and to say so (`shape_hint` coverage key) — see `docs/MCP-Communication-Efficiency-2026-09-04.md` for the measurements.
...
16. **Coverage signals (#116):** a data response states what it does NOT cover — one `_…_` line per signal directly under the H2/banner plus one `.optional()` typed key, emitted only when true: `field_limit_hit`, `provenance_omitted` (count + models), `isv_not_scanned`, `isv_excluded` (exact indexed `COUNT(*)` or nothing — never estimated), `partial_build` (every KB data tool, from `kb_metadata`). Every text comes from `coverageNotes()` in `shared.js` (static-scanned) and reaches the wire via `structuredResult(typed, md, format, { coverage })`; the keys live in `coverageKeys` (`output-schemas.js`) and MUST be spread into the tool's output schema first, or the SDK rejects the response. Rule #14 is the reason this exists: the default view drops per-row attribution, so the summary line is where that information survives. A 0-row `emptyResult` may carry the same lines as its note (`xref_find_references` does) — still a meta-response, no typed key.
...
**KB schema 1.2 (2026-09-03, #123/#124/#127)** — `build:kb` now also writes `objects_meta` (AxQuery, AxReport, AxMap, AxMacroDictionary, AxConfigurationKey, AxService, AxServiceGroup, AxSecurityPolicy, AxMenu — name, module, label, `properties_json`, never source; also indexed into `kb_search`), `forms.pattern` / `pattern_version` / `controls_count` + `form_controls` (the control tree: `@_i:type`, pattern, `DataSource`.`DataField`, parent), and `labels(label_id, language, text, label_file, module)` with an FTS5 `labels_fts` finalized post-save by `ensureLabelsFtsIndex()` (sql.js has no FTS5). `KB_LABEL_LANGUAGES=en-US,de,fr` (default `en-US`) is the language allow-list — all ~40 language folders would add ~16M rows (~1 GB). `mergeCustomKb()` merges `objects_meta`/`form_controls` only when both sides have them. `KB_NAMING_PREFIXES=TRB,iExt` (runtime, optional) turns on the prefix rule in `d365_preflight`.

`XREF_LIVE=1` (local stdio XRef only, #129): `src/local/xref-live-refresh.js` reads the ACTIVE XPP configuration from `HKCU\…\AX7\Development\Configurations\CurrentMetadataConfig` (the same key `Refresh-McpData.ps1` reads — never `.env`) and launches `build/update-xref-module.js` detached for the custom modules (`xref_module_sync`, else `model_versions.origin='custom'`, or `XREF_LIVE_MODULES=A,B`), logging to stderr because stdout is the transport. It is "fresh within minutes of session start", not a per-query SQL Server bridge — that follow-up stays on #129.

## Labels service (2026-09-08)

`d365fo-labels` (`/api/d365labels`, `src/local/mcp-server-labels.js`) serves **every label in every shipped language, always paired with its developer description** — the ` ;` comment line under a label (`Duty`, `[SecurityDuty FIM]`) that says what the label is for. Concept + measurements: `docs/Labels-Service-Concept-2026-09-08.md`. Own file `d365fo_labels.sqlite` (`LABELS_DB_PATH`), own builder `build/build-labels.js` (better-sqlite3, not sql.js — 76 language folders × ~383k ids): `label_meta` once per id (file, module, origin, description — measured identical across language files, so stored once), `labels` per (id, language), FTS5 `labels_fts` kept in sync by triggers, inventory tables, `labels_metadata`. Parser shared with the KB: `build/label-files.js` (`parseLabelFile` keeps the description the KB parser used to skip; `labelLines` is the KB's unchanged contract). Refresh rides the KB pipeline: `build:kb` calls `refreshLabelsAfterKb()` last on the CLI path only (`LABELS_SCAN=off` skips), `update-kb-model.js` re-ingests the compiled models (DELETE + INSERT — no INSERT-OR-IGNORE staleness), `Deploy.ps1 -Databases … labels` uploads. Four tools in `src/azure/labels-tools.js`: `labels_lookup` (ids → every language + description; case-tolerant through `label_meta`'s NOCASE index because `labels` is BINARY-indexed), `labels_search` (FTS, cursor), `labels_where_used` / `labels_for_object` — both read the **XRef** snapshot through `tryGetXrefDb()` (the KB dropped the label id at build time; XRef kept `Type/Object[/Element]?Property → /Labels/@id`, 558k refs) and return `errorResult('db-error')` when it is absent. tools/list 8,256 B (ceiling 8,420). `labels_lookup` is in `CORE_TOOLS`. **First real build (2026-09-08): 26,010,416 rows / 390,174 ids / 75 languages, 5.0 GB, 26.6 min; 78% of ids carry a description, 857 (0.28%) disagree across language files (first wins, count in `labels_metadata`).** Two traps that cost an hour each: language folders come in three casings (`en-US`/`en-us`/`en-Us` — `canonicalLanguage()` in `label-files.js` folds them, and a labels DB built before that fix needs the en-us rows merged), and the search SQL MUST keep its `CROSS JOIN` — with a `language` filter the planner otherwise drives from the 355k-row language slice and the call times out at 60 s (`labelsSearchSql()` is exported and its plan is asserted in `test/labels-tools.test.js`).

## Trace Insight — the MCP on the traces (2026-09-11)

Concept: `docs/ERP-Trace-Insight-Service-Concept-2026-09-11.md`. Two read tools on the **KB server** whose every answer is computed from trace records — `d365_prior_art` (request key or text → the cheapest answered run's call recipe with arguments, objects touched, cost, the waste it still paid) and `d365_entity_insight` (functional entity → vocabulary mapping, confirmed `sem_mappings`, key fields with KB type + label + Labels description, field usage heat from the traces, never-touched fields, request keys). Registered from the end of `registerKbTools()` (`src/azure/insight-tools.js`), **outside `CORE_TOOLS`**. Source: `d365fo_insight.sqlite` built by `npm run build:insight` (`build/build-insight.js`, better-sqlite3, seconds) from **every** local sink file in `~/.claude/mcp-trace` — hook stream + the four stdio servers (kb/xref/labels/sec), pending and `.sent` — because while Azure connector tracing (F4) is blocked the local sinks ARE the complete corpus; `INSIGHT_DB_PATH` → `~/.claude/d365fo_insight.sqlite` → `/home/data/d365fo_insight.sqlite` (`tryGetInsightDb()`; absent → `errorResult('db-error')` naming the build, never a crash). Hook/server twins are paired by argument SUBSET in a 10 s window (the report's rule — server args carry Zod defaults); waste classes `empty | not-found | error | repeat | unfollowed_page | unreplayable`; the `sql` argument of raw_sql calls is dropped by the builder. `readBuildDate()` knows `insight_metadata`. `insight_snapshot.not_covered` on every response names what no trace holds (Sec inside dossiers, Task Recorder, Azure connector calls, payloads, raw_sql text). Sixth server, Table Storage reader, metrics/compare tools and the OKF bundle were **deliberately not built** (concept §6–7): KPIs belong in the ClaudeTrace report (`analyze.js`, not yet extended), a `--pull` from Table Storage comes with F4. Deploy: `Deploy.ps1 -Databases insight`; `Rebuild-Provenance.ps1` runs the insight build after the KB build. Tests: `test/insight-build.test.js` (pure model on a fixture that carries every record shape) + `test/insight-tools.test.js`. **The human half is the ClaudeTrace report (sink v0.5.0, 2026-09-11):** `src/report/analyze.js` derives the same facts from the same records for `/report` — waste classes, exchange coverage, entity hit rate, `recipes` per request key, `entity_usage`, and `kpis` overall + by week / request key / service / tool (`d365_prior_art` is excluded from the call median, or the tool meant to cut calls would be punished by the metric counting them). Entity resolution there needs `src/contract/vocabulary.json` — `build/gen-trace-ingest.js` now copies `config/semantic-vocabulary.json` under the same `CONTRACT.sha256` manifest (`EXTERNAL_SOURCES`), so it cannot drift. The report also pairs hook/server twins in a **second pass without the 10 s window** inside one investigation: a batch of parallel calls lands the hook twin up to a minute late and the missed pair counted one call twice.

## Batching (issue #83)

`d365_get_enum` (`enum_names`), `d365_check_field_exists` (`tables`) and `xref_object_summary` (`object_names`) accept several targets per call. `xref_check_exists` (`objects[]`, 1..50 — `Name`, `/Type/Name` or `Owner.method`) and `sec_check_exists` (`roles`/`duties`/`privileges`/`entry_points`, ≤50 total, casing-tolerant with `canonical_name`) are **batch-only** preflights (#118): one target is a batch of one, and there is no singular alias. Two rules, both enforced by `test/batch-tools.test.js`:
