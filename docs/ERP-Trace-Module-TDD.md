...
| R11 | v1.3: `expected_entities` on `open` is optional; the hook path never names an entity at the start | The comparison across ERPs joins on the **logical** entity; if the dossier names it only once the investigation is over, the join is reconstructed. The prompt already carries it: Claude passes `functional_context: "vendor"` on every lookup, and the interpreted line names the concept. | `expected_entities` **required** on `open` (may be empty) with `entities_from`; the hook derives it from the first stamped call's `functional_context` and, failing that, from a vocabulary-alias match on `request.interpreted` (§5.3, §10, WI-16). Physical entities still fall out of `touched[]` / `primary_table` and get their role in `annotate`. A hypothesis, never a classification. |
...
phase: open
  investigation: { id, purpose: dashboard|report|app|migration_mapping|dq_rule|support|process_analysis|comparison|other,
                   ...
                   expected_entities: identifier[]≤20,                    // vocabulary ids — the LOGICAL entity hypothesis, always present (may be [])
                   entities_from: prompt|functional_context|vocabulary_match|none }   // how the hypothesis was obtained
...
- **The logical entity is named at `open`, the physical one as it is found.** `expected_entities` holds the vocabulary ids (`config/semantic-vocabulary.json`, ERP-neutral, e.g. `vendor`, `party`, `address`) the request is expected to touch — written **from the interpreted request, before the first call**, so the cross-ERP join key exists before any ERP-specific name does. Tool path: the skill requires it (an empty list is allowed, an omitted key is not). Hook path, in order of preference and recorded in `entities_from`: (1) `functional_context` — the skill already makes Claude pass a vocabulary id on the lookup tools, so the hook lifts the value(s) of the first stamped call(s) of the investigation (zero extra model discipline, already an id); (2) `vocabulary_match` — `request.interpreted` matched against the vocabulary's `entity_id`, `name` and an optional `aliases[]` (deterministic, needs no model cooperation, `vocabulary-match.js` in the contract package so the tool path can use it as a fallback too; the vocabulary carries no aliases today — adding them, e.g. `supplier` → `vendor`, `postal address` → `address`, is a vocabulary PR and a WI-16 prerequisite); (3) `prompt` when Claude named them itself; (4) `none`. The **physical** entity is not identified at the prompt: it falls out of the first call records (`touched[]`, the entity summary's `primary_table`) and receives its `role` / `functional_entity` / `counterpart` in `annotate`. `expected_entities` is Claude's guess from the prompt and `annotate` is what the investigation showed; the downstream comparison sees both, and the gap between them is itself a measurement (how often the initial guess was right).
...
| `src/trace/contract/vocabulary-match.js` (new) | `matchEntities(text, vocabulary) → identifier[]` — case-insensitive whole-word match of `request.interpreted` against `config/semantic-vocabulary.json` `entity_id` / `name` / optional `aliases[]` (none shipped yet — a WI-16 prerequisite), longest alias first, deduped, ≤ 20, in the order they appear in the text. Pure, no I/O; the vocabulary is injected (the hook reads the plugin's copy, the tool the semantic store's). Used by the hook when no `functional_context` was seen and by `trace_investigation.open` when Claude passed no `expected_entities`. |
| Plugin skill `d365fo-mcp-tooling` | Two paragraphs. **Claude Code (hooks installed):** "Tracing is automatic. Your interpretation line, each strategy line before a group of calls, each call and your final answer are recorded; tool responses are not. Write one strategy line before each group of calls; state the request ERP-neutrally in your first line. `annotate` remains the one tool you call yourself, when a table is identified as source/excluded, a field's correct shape is stated, or the user describes a process or lifecycle." **claude.ai / no hooks:** "`open` before the first lookup with `request.key` (`req.<domain>.<question>`, reuse it on another ERP), `request.interpreted` = your interpretation line, `approach`, `expected_entities` = the vocabulary ids the request touches (always — `[]` when none applies); `step` with your strategy line before each group of calls; `annotate` as above; `close` with your final answer as `summary` and `differences` when you have seen the other ERP's dossier. Pass `investigation_id` on data tools. Never paste data values or the user's raw text." |
...
- OK (entities at open): the vendor fixture's first stamped call carries `functional_context: 'vendor'` → the `open` record has `expected_entities: ['vendor']`, `entities_from: 'functional_context'` (the `open` is emitted at the first MCP `PreToolUse`, so the value is known); on a fixture whose calls carry no `functional_context`, the interpreted line *"…the vendor table and data entity structure…"* → `expected_entities: ['vendor']`, `entities_from: 'vocabulary_match'`; a line matching nothing → `[]`, `entities_from: 'none'`; `matchEntities()` on *"customer and vendor postal address"* → `['customer','vendor','address']` in text order, aliases (`supplier` → `vendor`) resolved, and never a substring hit (`vendors` matches, `vendorsitedesc` does not).
...
- [ ] **Logical entity at `open`** (§5.3, R11): `expected_entities` required (may be empty) + `entities_from`; hook derivation `functional_context` → `vocabulary_match` → `none`; `vocabulary-match.js` in the contract package.
