# Entra setup for the D365FO MCP services — admin handover

Everything the directory admin needs to create for the Entra OAuth cutover of
the D365FO MCP Function App (`tis-d-mcpd365fo-func`), and the full explanation
of what each object does and where its counterpart lives in this repo.

The code side is already implemented and deployed (dormant behind
`REQUIRE_AUTH=false`); once the objects below exist, the cutover is a single
script run. Full runbook: [`MCP-Entra-Auth-Setup.md`](MCP-Entra-Auth-Setup.md).

---

## The email to Aaron

> **Subject: App registration + security group for D365FO MCP (same shape as orion-mcp)**
>
> Hi Aaron,
>
> I'd like to put Entra OAuth in front of the D365FO MCP Function App
> (`tis-d-mcpd365fo-func`, tenant `0f861177-7722-4f06-8db9-3384e5321a9f`).
> It's the same shape as `sp-tis-p-orion-mcp` / `sp-tis-t-pulsar-mcp` —
> a single registration where the MCP server is both the resource and the
> public client (no OBO, no separate client app) — plus one app role.
> Could you set up the following two objects?
>
> **1. App registration `sp-tis-d-mcpd365fo-mcp`** (single tenant):
>
> - *Expose an API* → Application ID URI: **`api://sp-tis-d-mcpd365fo-mcp`**;
>   requested access token version **2** (manifest:
>   `api.requestedAccessTokenVersion: 2`).
> - Delegated scope **`user_impersonation`** (admins and users may consent),
>   display name e.g. "Access D365FO MCP as the signed-in user".
> - *App roles* → one role: display name **MCP Access**, value **`Mcp.Access`**
>   (exact casing — my code matches this string), allowed member types
>   **Users/Groups**.
> - *Authentication* → **Allow public client flows = Yes**; under *Mobile and
>   desktop applications* add redirect URIs
>   **`https://claude.ai/api/mcp/auth_callback`** and **`http://localhost`**
>   (same as orion-mcp).
> - *API permissions* → delegated permission to its own `user_impersonation`
>   scope, then **Grant admin consent**.
>
> **2. Security group `D365FO-MCP-Users`** (plain security group):
>
> - Initial member: me — Florian Dittgen,
>   oid `9495865f-c1c7-459f-87a6-4e9d8a20fb28`.
> - Enterprise application of the new registration → *Users and groups* →
>   assign this group to the **MCP Access** role.
> - Enterprise application → *Properties* → **Assignment required = Yes**.
>
> **What I need back:** just the registration's **Application (client) ID**
> and a confirmation of the App ID URI + group assignment. I run the Easy
> Auth cutover on the Function App myself.
>
> Background if useful: this is the pattern from your setup with Karl-Johan,
> minus the OBO hop — the Function App reads local SQLite, so it *is* the
> resource server and reads the `roles` claim straight from the incoming
> token.
>
> Thanks!
> Florian
...
> two derived values do: the **client ID** Aaron returns is the `-ApiAppId`
> parameter of `scripts/Enable-McpAuth.ps1` (mandatory, no default), and the
> **App ID URI** is `-AppIdUri` (default `api://sp-tis-d-mcpd365fo-mcp`,
> `Enable-McpAuth.ps1:28`). Both are written into the Function App's Easy
> Auth config as allowed audiences — the Node code never sees them.
...
> `Enable-McpAuth.ps1:32`) as
> `https://login.microsoftonline.com/<tenant>/v2.0` (`Enable-McpAuth.ps1:41`)
> — which is why token version **2** on the registration is mandatory. The
> **scope name** has no counterpart in this repo's server code; it resurfaces
> only in client configuration (runbook Part D) and the Copilot Studio
...
> (`decideMcpAuthorization`, enforced by a test in `test/mcp-auth.test.js` —
> `mcp.access` is rejected). Which claim types count as roles IS hard-coded:
> `typ === 'roles'` or `typ` ending in `/role` (`mcp-auth.js:62`) — the two
> forms Easy Auth emits.
...
| Scope `user_impersonation`, redirect URIs, group name/membership | Expose an API / Authentication / group | No server-side counterpart; scope reappears in client config (runbook Part D) + Copilot swagger | Entra only |
...
One operational note: role assignments are baked into tokens **at issue
time**. Someone added to `D365FO-MCP-Users` today still carries their old
role-less token until it expires — they'll see 403s until they sign out and
back in. Our 403 response text already tells the caller exactly that
(`mcp-auth.js:116`).
