...
|---|---|---|
| A request you may have investigated before (known `Request:` key, recurring ticket shape) | `d365_prior_art` with `request_key` (or `text`) — the cheapest answered run's call recipe, objects touched, cost, waste it still paid (from the traces; ~150 tokens on a miss) | Reuse the recipe in your strategy line; `d365_entity_insight` for the entity it names |
| A functional entity by business name (customer, vendor, sales order…) | `d365_entity_insight` — vocabulary mapping + key fields with type/label/description + what investigations actually touched (default `sections: summary,keys,usage`, ≤ ~1.5k tokens) | `sections: ["fields"]` for the field heat map and never-touched fields; then the dedicated tools for detail |
...
6. **raw_sql: aggregate, then ≤ 5 rows per group.** Never a > 50-row dump. A `raw_sql` call also never reaches the trace sink — its `sql` argument fails the privacy scan and the record is dead-lettered (9 of 9 on 2026-09-10), so an investigation done in SQL leaves a dossier with holes; the dedicated tools are the ones that get recorded.
...
- **Skill versions.** Skills are snapshotted per session; after a plugin change run `/plugin marketplace update`, `/plugin update`, then start a new session. The cache is keyed by `plugin.json` version — a skill or hook change without a version bump is never picked up.
- **Two Azure signals that look like faults and are not** (2026-09-10): for ~2 minutes after a Function App publish, `/health` answers 404 and App Insights logs "No job functions found" — that is the host restart; retry with delays before diagnosing. And `az storage … --auth-mode key` reports "storage account not found" when the real error is `AuthorizationFailed`, i.e. a lapsed PIM role — `az group show` on the resource group shows the true error class, and `az login` does not help until the role is active again.
...
## 8. Trace protocol (hook-captured since 2026-09-07; KB, XRef and — since plugin 1.7.1 — Labels; Security is deliberately never hook-captured)

Every D365 KB/XRef/Labels investigation in Claude Code is recorded as a cross-ERP trace: the request as **you** interpreted it, your strategy lines, every MCP call with its replayable arguments (never the response), the final answer. The hook reads it all from the transcript — you only have to write three things well:
...
- **The strategy lines are what make a dossier readable** (measured 2026-09-10 on the first five recorded investigations): three of the five had zero strategy lines, so in the Trace Dossiers report every call collapsed under the opening record and the exchange structure — *this line of thought → these calls → this result* — was lost. One line per group of calls costs ~15 tokens and is the only thing that turns a call list into an investigation.
- **The traces read back (2026-09-11, `d365_prior_art` / `d365_entity_insight` on the KB server).** Both compute every answer from the trace snapshot `d365fo_insight.sqlite` (`npm run build:insight` from `~/.claude/mcp-trace`, all streams, all four servers, pending + `.sent`). Every response states what the traces do NOT cover (Sec inside dossiers, Task Recorder, Azure connector calls, payloads, raw_sql text) — read a recipe as partial when that matters. Your `Request:` key is what makes prior art findable next time: keep it stable across sessions.
- **Reading the traces.** Live report: `https://tis-d-claudetrace-func.azurewebsites.net/report?code=<function key>` (`/report/data` is the JSON behind it, `?days=7|30|90|all`); offline: `npm run page -- --ndjson ~/.claude/mcp-trace` in the ClaudeTrace repo. The sink only holds what was pushed — the stdio servers write locally and `scripts/Push-LocalTraces.ps1` (MCP repo) ships the files; on 2026-09-10 the report showed 34 calls until the push made it 103. In the raw files expect two records per call on a stdio session: the hook's (knows the investigation) and the server's (knows snapshot date, duration, touched objects, and carries the Zod-filled defaults the hook never saw) — the report pairs them on argument subset.
