...
MCP server key: `d365kb` · 30 tools · all read-only (`readOnlyHint`), idempotent, closed-world.
...
| `d365_prior_art` | Has this request been investigated before? |
| `d365_entity_insight` | One functional entity (id, name or alias) in one call: its D365FO data entities, tables and key fields, confirmed mappings, field meaning from the KB and Labels |
...
Has this request been investigated before? By request_key (exact) or text (terms against recorded request keys, interpretations and entity aliases): the cheapest answered run's call recipe with arguments, objects touched, cost, and the waste it still paid. Call first on a known request key.

| Param | Type | Required | Description |
|---|---|---|---|
| `request_key` | string (min 2, max 200) | no |  |
| `text` | string (min 3, max 300) | no | Used when request_key is absent. |
| `limit` | integer (≥1, ≤10) | default `3` |  |
| `format` | `markdown` \| `toon` \| `auto` | default `"auto"` | auto (default) = smallest; markdown to quote verbatim. |

## `d365_entity_insight`

One functional entity (id, name or alias) in one call: its D365FO data entities, tables and key fields, confirmed mappings, field meaning from the KB and Labels, and what real investigations touched of it (usage heat, never-touched fields). sections: summary, keys, usage (default), fields, recipes.

| Param | Type | Required | Description |
|---|---|---|---|
| `entity` | string (min 2, max 100) | yes |  |
| `sections` | array<`summary` \| `keys` \| `usage` \| `fields` \| `recipes`> | no |  |
| `languages` | array<string (min 2, max 10)> | no | Label languages; default en-US. |
| `limit` | integer (≥1, ≤200) | default `30` | Rows in usage / fields. |
| `format` | `markdown` \| `toon` \| `auto` | default `"auto"` | auto (default) = smallest; markdown to quote verbatim. |
