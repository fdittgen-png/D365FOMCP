...
MCP server key: `d365sec` · 19 tools · all read-only (`readOnlyHint`), idempotent, closed-world.
...
| `sec_check_exists` | Preflight: do these roles/duties/privileges/entry points exist (case-insensitive, ≤50 total)? |
...
Full-text search across roles, duties, privileges and users. Scope with `modules` to specific models; users carry no module and are excluded when the filter is set.
...
Preflight: do these roles/duties/privileges/entry points exist (case-insensitive, ≤50 total)? Returns the canonical spelling; misses get suggestions.

| Param | Type | Required | Description |
|---|---|---|---|
| `roles` | array<string (min 1, max 500)> | no | Role ids or names. |
| `duties` | array<string (min 1, max 500)> | no | Duty ids or names. |
| `privileges` | array<string (min 1, max 500)> | no | Privilege names. |
| `entry_points` | array<string (min 1, max 500)> | no | Entry point names. |
| `format` | `markdown` \| `toon` \| `auto` | default `"auto"` | auto (default) = smallest; markdown when quoting verbatim. |
