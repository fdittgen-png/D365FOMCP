...
23. **Write the trace protocol lines** (2026-09-07, tightened 2026-09-08): a `Request:` line (ERP-neutral, no names or values) and an `Entities:` line (business terms) as the FIRST text of the reply — a block written between tool calls may never reach the transcript — then one strategy line per group of calls — the hook turns them into the investigation's `open` and `step` records; a group of calls without a line has no step. Never put party data in a strategy line: that line is dropped from the trace. In the final answer, `Entity: <kind> <Name> as <role> [= <functional_entity>] [~ <erp>:<Name>]` and `Expect: <ids>` lines become the dossier's `annotate` record (plugin 1.5.0).
...
25. **After an Azure code deploy, trust `/api/ping` 200 and the host's function count — never the 401 checks** (2026-09-07): a package missing one imported `src/<dir>` makes the worker index zero functions while Easy Auth keeps answering 401 to every authenticated probe; the staging list of the deploy scripts is explicit and guarded by `test/deploy-staging.test.js`. Restore line-ending-only diffs (`git diff --ignore-all-space --stat`) before committing — a mis-quoted `sed -i` over `scripts/*.ps1` touched 12 files without changing a character.
...
26. **One strategy line per group of calls, or the dossier has no shape** (2026-09-10): in the Trace Dossiers report, an investigation without strategy lines is one opening record followed by every call — three of the first five recorded investigations looked like that. The line is what pairs *intent* with *calls*; write it before the group, ~15 tokens.
27. **`raw_sql` is invisible to the trace** (2026-09-10): the sink's privacy scan dead-letters every record whose `sql` argument carries a term, which was 9 of 9 raw_sql records on the first full push. Prefer the dedicated tools when the investigation should be replayable against another ERP; if SQL is unavoidable, say what it established in the strategy line, which is recorded.
28. **Two Azure signals that are not faults** (2026-09-10, extends rule 25): a 404 on `/health` plus "No job functions found" for ~2 minutes after a publish is the host restart — retry with delays, then read `/admin/functions`; "storage account not found" from `az storage … --auth-mode key` is `AuthorizationFailed` in disguise — check the PIM role first (`az group show` on the resource group shows the real class), and expect `az login` to change nothing until the role is active.

---

*Version: 2.4 | Date: 2026-09-10 | Added: cost rules 26–28 (strategy line per call group after 3 of 5 recorded investigations had none; raw_sql records are dead-lettered by the sink's privacy scan; post-publish 404 and PIM-lapse "not found" are not faults) from the first read of the Trace Dossiers report (ClaudeTrace v0.4.0, `/report`).*
